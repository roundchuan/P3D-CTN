...
#                      interpolation=cv2.INTER_LINEAR)
  ...
  def next_batch(self, batch_size, depth):
    ...
    for i in xrange(batch_size):
      ...
      all_pred = np.load('data/jhmdb/tpn_119/{}/bboxes_nms_74.npy'.format(video['video_name'])) 
  ...
  def next_val_video(self):
    ...
    all_pred = np.load('data/jhmdb/tpn_119/{}/bboxes_nms_74.npy'.format(self._vddb[self._curr_idx]['video_name']))
  ...
  def next_rec_video(self):
    ...
    gt_label = self._vddb[self._curr_idx]['gt_label']
    vid_name = self._vddb[self._curr_idx]['video_name']
    pred = np.load('data/jhmdb/tpn_119/{}/bboxes_nms_74.npy'.format(vid_name)) * 1.25
    self._curr_idx += 1
    return video, \
           gt_bboxes, \
           gt_label, \
