...
def apply_expand(imglist, tubes, expand_param, mean_values=None):
    ...
    if random.random() < expand_param['expand_prob']:
        ...
        if mean_values is not None:
            for i in xrange(len(imglist)):
                out_imglist[i] += mean_values
        for i in xrange(len(imglist)):
            out_imglist[i][h_off:h_off+oh, w_off:w_off+ow, :] = imglist[i]
    
        #for ilabel in tubes:
        for itube in xrange(len(tubes)):
            out_tubes[itube] += np.array([w_off, h_off, w_off, h_off], dtype=np.float32)
...
class jhmdb_au():
  ...
  def get_anchors(self):
    base_anchors = np.load(
      self._data_path + '/cache/anchors_8_16.npy').transpose()
  ...
  def next_adv_batch(self, batch_size, depth):
  ...
  def next_batch(self, batch_size, depth):
    ...
    for i in xrange(batch_size):
      ...
      f_idx = int(video['gt_bboxes'][curr_frame, 0])
      tmp_video = video['video'][f_idx : f_idx + depth] - self._mean_frame
     # tmp_video = tmp_video.astype(float)
      tmp_bbox = video['gt_bboxes'][curr_frame : curr_frame + depth, 1 : 5]
...
#      tmp_video, tmp_bbox = apply_expand(tmp_video, tmp_bbox, expand_params, mean_values=[0,0,0])
      batch_video[i] = tmp_video
      batch_label[i] = video['gt_label']
      batch_vid_name[i] = video['video_name']
      batch_bboxes[i] = tmp_bbox
  ...
  def next_rec_video(self):
    ...
    gt_label = self._vddb[self._curr_idx]['gt_label']
    vid_name = self._vddb[self._curr_idx]['video_name']
    pred = np.load('data/jhmdb/tpn_119/{}/bboxes_au_46.npy'.format(vid_name)) * 1.25
    self._curr_idx += 1
    return video, \
           gt_bboxes, \
           gt_label, \
