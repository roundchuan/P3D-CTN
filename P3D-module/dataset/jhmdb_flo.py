...
class jhmdb_flo():
  def __init__(self, name, clip_shape, split=1):
    self._name = name
    self._data_path = 'data/jhmdb_flo'
    self._vddb = []
  ...
  def _load_annotations(self, video):
    ...
    """
    gt_file = os.path.join('data/jhmdb/puppet_mask',
  ...
  def clip_reader(self, video_prefix):
    ...
    clip = []
    r1 = 0
    framepath = os.path.join('/data/wjc/caffe_act1/data/JHMDB/Flownet2_flow', video_prefix)
    num_frames = len(glob.glob(framepath + '/*.flo'))
    for i in xrange(num_frames):
      filename = os.path.join(
          '/data/wjc/caffe_act1/data/JHMDB/Flownet2_flow', video_prefix,
          '%05d.flo' % (i + 1))
...
#                      interpolation=cv2.INTER_LINEAR)
  ...
  def next_batch(self, batch_size, depth):
    ...
    batch_pred = np.empty((batch_size, depth, 40, 4))
    is_last = False
    for i in xrange(batch_size):
      ...
      all_pred = np.load('data/jhmdb/tpn_119/{}/bboxes_au_46.npy'.format(video['video_name'])) 
  ...
  def next_val_video(self):
    ...
    all_pred = np.load('data/jhmdb/tpn_119/{}/bboxes_au_46.npy'.format(self._vddb[self._curr_idx]['video_name']))
  ...
  def next_rec_video(self):
    ...
    gt_label = self._vddb[self._curr_idx]['gt_label']
    vid_name = self._vddb[self._curr_idx]['video_name']
    pred = np.load('data/jhmdb_flow/tpn_119/{}/bboxes_au_46.npy'.format(vid_name)) * 1.25
    self._curr_idx += 1
    return video, \
           gt_bboxes, \
           gt_label, \
