...
class ucfsports_flo():
  def __init__(self, name, clip_shape, split=1):
    self._name = name
    self._data_path = 'data/ucfsports_flo'
    self._vddb = []
    ...
    self.videos = [l.split() for l in file(os.path.join('/data/wjc/action-faster-rcnn/ucfsports-anno/videos.txt'))]
    self.test_videos = [v[0] for v in self.videos if v[2]=="test"]
    self.train_videos = [v[0] for v in self.videos if v[2]=="train"]
    self.video_to_label = {v[0]: self._class_to_ind[v[1]] for v in self.videos }


    with open("/data/wjc/caffe_act1/cache/UCFSports-GT.pkl", "rb") as fgt:
      self._gt_file = cPickle.load(fgt)
    cache_file = os.path.join(self._data_path, 'cache',
        'ucfsports_%d_%d_db_v2.pkl' % (self._height, self._width))
...
#      raise Exception(gt_file + 'does not exist.')
   ...
   # masks = sio.loadmat(gt_file)['part_mask']
    #print(gt_file)
   # gt_label = self.video_to_label[video['video_name'][: video['video_name'].find("/")]]
    gt_label = self.video_to_label[video['video_name']]
   # print (gt_label)
    masks = self._gt_file['gttubes'][video['video_name']][gt_label-1]
 #   masks = self._gt_file['gttubes'][video['video_name']][gt_label]
    num_objs = len(masks)
    depth = len(masks[0])

    ratio, pixels = self.clip_reader(video['video_name'])

    gt_bboxes = np.zeros((depth, num_objs, 5), dtype=np.float32)
    for j in xrange(depth):
      for ix in range(num_objs):
        tmpbox = np.array([masks[ix][j,0]-1, masks[ix][j,1] * ratio[1], masks[ix][j,2] * ratio[0], masks[ix][j,3] * ratio[1], masks[ix][j,4] * ratio[0]])
        gt_bboxes[j,ix,:] = tmpbox
#    for j in xrange(depth):
#      mask = masks[:, :, j]
#      (a, b) = np.where(mask > 0)
#      y1 = a.min()
...
#      x2 = b.max()
 ...
 #     gt_bboxes[j] = np.array([masks[0][j,0]-1, masks[0][j,1] * ratio[1], masks[0][j,2] * ratio[0],
  #                             masks[0][j,3] * ratio[1], masks[0][j,4] * ratio[0]])
 ...
 #     file_name = os.path.join('data/jhmdb/splits',
  ...
  #      tmp.append(video_name)
    ...
    for ll in xrange(1, 151):
      split = np.zeros(3, dtype=np.uint8)
      if ll < 5:
        split[0] = 2
      else:
        split[0] = 1         
      vddb.append({'video_name': str(ll).zfill(3),
                   'split': split}) 
  ...
  def clip_reader(self, video_prefix):
    ...
    clip = []
    r1 = 0
    framepath = os.path.join('/data/wjc/caffe_act1/data/UCFSports', 'Flownet2_flow', video_prefix)
#    print (framepath)
    num_frames = len(glob.glob(framepath + '/*.flo'))
    for i in xrange(num_frames):
      filename = os.path.join(
          '/data/wjc/caffe_act1/data/UCFSports', 'Flownet2_flow', video_prefix,
          '%06d.flo' % (i + 1))
...
#      print (filename)
 ...
 #     clip.append(im)
  ...
  def next_batch_tpn(self, batch_size, depth):
    ...
    """
    batch_video = np.empty((batch_size, depth, self._height, self._width, 2))
    ...
    for i in xrange(batch_size):
      ...
      print (video['gt_bboxes']) 
      total_frames = video['gt_bboxes'].shape[0]
      curr_frame = np.random.randint(0, total_frames - depth)
 ...
 #     batch_pred[i] = tmp_pred
  ...
  def next_batch(self, batch_size, depth):
    ...
    for i in xrange(batch_size):
      ...
      all_pred = np.load('data/ucfsports/tpn_40_s1/{}/bboxes.npy'.format(video['video_name'])) 
      tmp_pred = all_pred[:, curr_frame : curr_frame + depth]
      ...
      batch_label[i] = video['gt_label']
      batch_vid_name[i] = video['video_name']
      batch_bboxes = tmp_bbox.transpose(1, 0, 2)
      #print ('batch')
...
#    all_pred = np.load('data/jhmdb/tpn_6_8/{}/bboxes_925.npy'.format(self._vddb[self._curr_idx]['video_name']))
    all_pred = []
  ...
  def compute_mean_frame(self):
    sum_frame = np.zeros((self._height, self._width, 2), dtype=np.float32)
    num_frames = 0
  ...
  def cluster_bboxes(self, length=8, anchors=9):
    ...
    for db in self._vddb:
      ...
      for i in xrange(l):
        for j in xrange(boxes.shape[1]):
          if not(boxes[i,j, 0] + length == boxes[i + length - 1,j, 0] + 1):
            print('Invalid boxes!')
            continue
          curr = np.mean(boxes[i : i + length,j, 1 : 5], axis=0)
          x = (curr[2] - curr[0]) / 16
          y = (curr[3] - curr[1]) / 16
