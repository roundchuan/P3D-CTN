...
class ucfsports_v2():
  def __init__(self, name, clip_shape, split=1):
    self._name = name
    self._data_path = 'data/ucfsports'
    self._vddb = []
...
#      x2 = b.max()
 ...
 #     file_name = os.path.join('data/jhmdb/splits',
  ...
  def clip_reader(self, video_prefix):
    ...
    clip = []
    r1 = 0
    framepath = os.path.join('/data/wjc/caffe_act1/data/UCFSports', 'Frames', video_prefix)
#    print (framepath)
    num_frames = len(glob.glob(framepath + '/*.jpg'))
    for i in xrange(num_frames):
      filename = os.path.join(
          '/data/wjc/caffe_act1/data/UCFSports', 'Frames', video_prefix,
          '%06d.jpg' % (i + 1))
