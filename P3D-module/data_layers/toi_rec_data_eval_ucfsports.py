...
import caffe
from dataset.ucfsports import ucfsports
import numpy as np
...
class RecDataLayer():
  def __init__(self, net, model):
    ...
    self._height = 300
    self._width = 400
    self.dataset = ucfsports('val', [self._height, self._width], split=1)
  ...
  def forward(self):
    ...
    self._net.blobs['tois'].reshape(self._batch_size * 2392, 5)

    [clip, gt_bboxes, labels, all_pred, _, is_last] = self.dataset.next_val_video()

    n = int(clip.shape[0] )

    result = np.empty((n-self._depth+1, 2392, 22))
...
#      pred_anchors = np.reshape(pred, (-1,4)) * 1.25/ 16
      ...
      batch_tois = np.hstack((np.zeros((2392, 1)), self.anchors))
