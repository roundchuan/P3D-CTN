...
from dataset.jhmdb_flo_au import jhmdb_flo_au
import numpy as np
...
class RecDataLayer(caffe.Layer):
  def setup(self, bottom, top):
    ...
    #                         '/home/rhou/JHMDB')
    self.dataset = jhmdb_flo_au('train', [self._height, self._width], split=1)
...
#      gt_bboxes = np.array(np.mean(box, axis=1)) / 16
      ...
      overlaps = bbox_overlaps(
        np.ascontiguousarray(pred_anchors, dtype=np.float),
        np.ascontiguousarray(gt_bboxes, dtype=np.float))
      max_overlaps = overlaps.max(axis=1)
      gt_argmax_overlaps = overlaps.argmax(axis=0)

      curr_labels = np.ones(pred_anchors.shape[0]) * (-1)
 ...
 #     print inds.shape
      curr_bboxes = np.hstack((np.ones((len(inds), 1)) * i, pred_anchors[inds]))
