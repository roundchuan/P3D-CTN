...
#    print impath
    impath = os.path.dirname(im_file)
    im_name = os.path.basename(im_file)
    n = int(im_name.split('.')[0])
    score_path = '/data/wjc/TCNN_STCNN/data/jhmdb/tpn_cls'+impath[38:]+'/score.npy'
    scoref_path = '/data/wjc/TCNN_STCNN/data/jhmdb/tpn_cls'+impath[38:]+'/score_flow.npy'
    roi_path = '/data/wjc/TCNN_STCNN/data/jhmdb/tpn_119'+impath[38:]+'/bboxes_au_46.npy'
    #roi_all = np.load(roi_path)
    #score_all = np.load(score_path)
    #scoref_all = np.load(scoref_path)
    #scores_o = (score_all[n-1] + scoref_all[n-1]) * 0.5
    #rois_n = roi_all[n-1]*16
#    scores, boxes = im_detect(net, im, rois_n)
